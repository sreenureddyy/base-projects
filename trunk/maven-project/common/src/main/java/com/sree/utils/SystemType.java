...
public class SystemType {
	...
	public static boolean is32bit() {
		return (SystemProperties.getOSArch().compareTo("i386") == 0)
				|| (SystemProperties.getOSArch().compareTo("sparc") == 0)
				|| (SystemProperties.getOSArch().compareTo("x86") == 0)
				|| (SystemProperties.getOSArch().compareTo("ppc") == 0);
	}

	public static boolean is64bit() {
		return (SystemProperties.getOSArch().compareTo("amd64") == 0)
				|| (SystemProperties.getOSArch().compareTo("sparcv9") == 0)
				|| (SystemProperties.getOSArch().compareTo("ppc64") == 0)
				|| (SystemProperties.getOSArch().startsWith("IA64"));
	}
}